...
public class MainUDPTimeServer {
	...
	public static boolean handleRequests = false, threadOneBusy = false, threadTwoBusy = false;
	int portNumber;
	String IPAddress;
		...
		public void run(){
			...
			try{
				...
				if(query.equals("getDateAndTime")){
					System.out.println("From " + Thread.currentThread().getName() + ": Request received for Date and Time");
					...
					System.out.println("From " + Thread.currentThread().getName() + ": Date and Time Sent!");
		...
		public void run() 
		{
			...
			try{
				...
				while(true){
					...
					Thread worker = new ServerHandler(port, IPAddress, serverSocket, receivePacket, query);  
